...
class WxPay(http.Controller):
    ...
    def __init__(self):
        ...
        self.wx_pay = WeixinPay(self.app_id, self.mch_id, self.mch_key, self.notify_url)
    ...
    def login(self, param, **kwargs):
        ...
        url = self.wx_login.authorize(callback, "snsapi_base")  # 引导用户跳转到授权页面，callback = redirect_uri
    ...
    def pay_create(self, **kwargs):
        ...
        try:
            ...
            raw = self.wx_pay.jsapi(openid=openid, body=u"测试", out_trade_no=out_trade_no, total_fee=1, spbill_create_ip=remote_addr)  # 外部传入openid
